...
// We only import stripe for Android usage
...
const PurchaseScreen = () => {
  ...
  // We only init stripe for Android usage
  const { initPaymentSheet, presentPaymentSheet } = useStripe();
  const dispatch = useDispatch();

  // For iOS: keep track of IAP initialization
  ...
  // For iOS: set up in-app purchases (only do this once)
  useEffect(() => {
    if (Platform.OS === 'ios') {
      const connectToStore = async () => {
        try {
          ...
          console.log('IAP connection error:', error);
        }
      };
  ...
  const enrollIfFree = useCallback(async () => {
    try {
  ...
  }, [dispatch, courseId, navigation, showAlert]);

  /**
   * iOS in-app purchase flow using expo-in-app-purchases
   */
  const handlePurchaseIos = async () => {
    try {
      if (!iapConnected) {
        showAlert('Error', 'In-app purchase not connected to App Store.', [], 'alert-circle');
        setButtonLoading(false);
        return;
      }
      // 1. Get the product(s) from Apple (the ID must match what's set in App Store Connect).
      //    Suppose each course has an Apple productId. e.g. "com.myapp.course.<courseId>"
      //    For simplicity, let's say we have a single product ID. This must match your App Store Connect listing.
      const productId = `com.myapp.course.${courseId}`; 
      ...
      if (responseCode !== InAppPurchases.IAPResponseCode.OK || results.length === 0) {
        showAlert('Error', 'Could not fetch product info from Apple.', [], 'alert-circle');
        setButtonLoading(false);
        return;
      }

      // 2. Listen for purchase updates
      const subscription = InAppPurchases.purchaseUpdatedListener(async (purchase) => {
        // Called when the purchase completes
        if (purchase.responseCode === InAppPurchases.IAPResponseCode.OK) {
          // Payment success
          // In a real app, you'd verify receipts on server
          // Then enroll user
          try {
            ...
            showAlert('Enrollment Failed', error.message, [], 'alert-circle');
          }
        } else if (purchase.responseCode === InAppPurchases.IAPResponseCode.USER_CANCELED) {
          showAlert('Cancelled', 'Purchase was cancelled.', [], 'alert-circle');
        } else {
          showAlert('Error', 'Purchase failed. Please try again.', [], 'alert-circle');
        }
      ...
      // 3. Request the purchase
      await InAppPurchases.purchaseItemAsync(productId);
    } catch (error) {
      setButtonLoading(false);
      showAlert('Purchase Error', error.message, [], 'alert-circle');
    }
  };

  /**
   * Android purchase flow using Stripe (like your old logic)
   */
  const handlePurchaseAndroid = async () => {
    try {
  ...
  const handlePurchase = async () => {
    ...
    // If paid:
    if (Platform.OS === 'ios') {
      await handlePurchaseIos();
    } else {
      // Android
      await handlePurchaseAndroid();
    }
  };
  ...
  return (
    <View style={[styles.safeArea, { backgroundColor: currentTheme.backgroundColor }]}>
      ...
      >
        ...
        <View
          style={[styles.heroWaveShape, { backgroundColor: currentTheme.backgroundColor }]}
        />
      </LinearGradient>
      ...
      >
        ...
        <View style={[styles.mainCard, { backgroundColor: currentTheme.cardBackground }]}>
          ...
          <Text
            style={[styles.courseTitle, { color: currentTheme.textColor }]}
            numberOfLines={2}
          >
            {title}
          </Text>
          ...
          <View style={styles.ratingRow}>
            ...
            <Text style={[styles.ratingNumber, { color: currentTheme.textColor }]}>
              {rating}
            </Text>
          </View>
          ...
          {price > 0 && (
            <>
              {course.saleEnabled && course.salePrice < price ? (
                ...
                <Text
                  style={[styles.normalPrice, { color: currentTheme.primaryColor }]}
                >
                  ${price.toFixed(2)}
                </Text>
              )}
